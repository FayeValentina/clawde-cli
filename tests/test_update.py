...
def test_update_status_does_not_require_npm(monkeypatch):
    monkeypatch.setattr(update_cmd, "_probe_openclaw", lambda: (True, None))
    monkeypatch.setattr(update_cmd, "_get_current_version", lambda: "openclaw 1.2.3")
    monkeypatch.setattr(
        update_cmd,
    ...
    def fail_if_called():
        raise AssertionError("_probe_npm should not be called for --status")

    monkeypatch.setattr(update_cmd, "_probe_npm", fail_if_called)
...
def test_update_repairs_broken_openclaw_install(monkeypatch):
    version_calls = iter(["unknown", "openclaw 1.1.0"])
    install_called = False
    ...
    monkeypatch.setattr(
        update_cmd,
        "_probe_openclaw",
        lambda: (False, "OpenClaw command is present but failed to run: broken install"),
    )
    monkeypatch.setattr(update_cmd, "_probe_npm", lambda: (True, None))
    monkeypatch.setattr(update_cmd, "_get_current_version", lambda: next(version_calls))
    ...
    assert "repair it" in result.stdout
...
def test_update_skips_wait_when_gateway_reinstall_fails(monkeypatch):
    version_calls = iter(["openclaw 1.0.0", "openclaw 1.1.0"])
    wait_called = False
    ...
    monkeypatch.setattr(
        update_cmd,
        "_build_gateway_status_summary",
        lambda: ("degraded", "runtime not running"),
    )
    monkeypatch.setattr(update_cmd, "_install_latest_openclaw", lambda: (True, "updated"))
    monkeypatch.setattr(
        update_cmd,
        "_install_gateway_service",
        lambda: (False, "openclaw gateway install failed: bad launchagent"),
    )
    ...
    assert result.exit_code == 0
    assert wait_called is False
    assert "Skipping readiness wait" in result.stdout
